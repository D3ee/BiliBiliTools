...
class login():
    ...
    async def calc_sign(self,str):
        str = str + "25bdede4e1581c836cab73a48790ca6e"
        hash = hashlib.md5()
    ...
    async def get_pwd(self, username, password):
        ...
        temp_params = 'appkey=07da50c9a0bf829f'
        sign = await self.calc_sign(temp_params)
        params = {'appkey': '07da50c9a0bf829f', 'sign': sign}
    ...
    async def login(self):
        ...
        temp_params = 'appkey=07da50c9a0bf829f&password=' + pwd + '&username=' + user
        sign = await self.calc_sign(temp_params)
...
class judge(login):
    ...
    async def getsubmit_video(self):
        ...
        for mid in top50_attention_list:
            url = "https://api.bilibili.com/x/space/arc/search?mid=" + str(mid) + "&pn=1&ps=50&jsonp=jsonp"
            response = requests.get(url)
            temp=response.json()
            datalen = len(temp['data']['list']['vlist'])
            time.sleep(0.2)
            for i in range(0, datalen):
                aid = temp['data']['list']['vlist'][i]['aid']
    ...
    async def share(self):
        ...
        ts = CurrentTime()
        temp_params = "access_key="+login.access_key+"&aid="+str(aid)+"&appkey=07da50c9a0bf829f&build=5260003&from=7&mobi_app=android&platform=android&ts="+str(ts)
        sign = await self.calc_sign(temp_params)
        data = {
